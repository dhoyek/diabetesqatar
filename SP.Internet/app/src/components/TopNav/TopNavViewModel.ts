        ...
        if (!termSetId) {
            ...
            this.taxonomyModule.init().then(() => {
                ...
                this.taxonomyModule.getNavigationTaxonomyNodes(new SP.Guid(termSetId)).then((navigationTree: ITaxonomyNavigationNode[]) => {
                    ...
                    let currentNode = this.utilityModule.getNodeByUrl(navigationTree, window.location.pathname);
                    if (currentNode !== undefined) {
                        if (currentNode.ParentId !== null) {
                            const currentNodeParent = this.utilityModule.getNodeByTermId(navigationTree, currentNode.ParentId);
                            if (currentNodeParent.ParentId !== null) {
                                const parentNode = this.utilityModule.getNodeByTermId(navigationTree, currentNodeParent.ParentId);
                                if (parentNode.ChildNodes.length > 0) {
                                    navigationTree = parentNode.ChildNodes;
                                }
                            }
                            else
                                if (currentNodeParent.ChildNodes.length > 0) {
                                    navigationTree = currentNodeParent.ChildNodes;
                                }
                        }
                    }
                    this.initialize(navigationTree);
